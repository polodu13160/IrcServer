...
void	User::whoCmd(Server &server, const std::vector<std::string>& arg){
	...
	if(arg.size() == 0){
		...
		if(!channel){
			...
			const std::string	line = ":" + server._ip + " 403 " + this->getNickname() + " " + arg[0] + " :No such channel\r\n";
		...
		// RPL_ENDOFWHO (315)
		const std::string line = ":" + server._ip + " 315 " + this->getNickname() + " " + channel->getName() + " :End of /WHO list\r\n";
		Server::sendCheck(this->getUserFd(), line.c_str(), line.size(), 0);
	}
}
